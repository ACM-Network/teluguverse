...
export default function SearchPage() {
  ...
  const buildQuery = useCallback(() => {
    ...
    p.set('limit', '24')
    return p.toString()
  }, [debouncedQuery, type, sort, year, language, ott, rating, dubOnly, page])

  useEffect(() => {
    setLoading(true)
    setPage(1)
  }, [debouncedQuery, type, sort, year, language, ott, rating, dubOnly])
  ...
  return (
    <div className="min-h-screen bg-dark pt-24 pb-16">
      <div className="container-tv">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-cinzel text-3xl font-black text-white mb-1">
            {debouncedQuery ? `Results for "${debouncedQuery}"` : type ? `${type.charAt(0)+type.slice(1).toLowerCase()} Library` : 'Explore Everything'}
          </h1>
          <p className="font-telugu text-gray-500 text-sm">{total.toLocaleString()} titles found • TeluguVerse Database</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search movies, anime, series... / సినిమాలు వెతకండి..."
            className="w-full bg-surface border border-border focus:border-yellow-400/50 rounded-xl py-4 pl-11 pr-4 text-white font-rajdhani text-base outline-none transition-all" />
          {query && <button onClick={() => setQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white">✕</button>}
        </div>
        ...
        <div className="flex gap-2 flex-wrap mb-8">
          {[{ v:'',l:'All' },{ v:'MOVIE',l:'🎬 Movies' },{ v:'ANIME',l:'⚡ Anime' },{ v:'SERIES',l:'📺 Series' },{ v:'KDRAMA',l:'🌸 K-Drama' },{ v:'CARTOON',l:'🎭 Cartoon' },{ v:'HOLLYWOOD',l:'🎪 Hollywood' }].map(t => (
            <button key={t.v} onClick={() => setType(t.v)}
              className={`px-4 py-2 rounded-lg text-xs font-bold font-rajdhani tracking-wide transition-all border ${type===t.v?'bg-yellow-500/20 border-yellow-500/50 text-yellow-400':'bg-surface border-border text-gray-400 hover:border-yellow-400/30 hover:text-yellow-400'}`}>
              {t.l}
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            <motion.div key="empty" initial={{ opacity:0 }} animate={{ opacity:1 }} className="text-center py-24">
              ...
              <button onClick={() => { setQuery(''); setType(''); setYear(''); setLanguage(''); setOtt(''); setRating(''); setDubOnly(false) }}
