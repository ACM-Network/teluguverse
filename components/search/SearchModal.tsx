    ...
    <AnimatePresence>
      {isSearchOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          style={{ background: 'rgba(5,6,12,0.88)' }}
          ...
          <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            ...
            <div className="relative mb-5 max-w-3xl mx-auto">
              ...
              <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={handleKeyDown}
                ...
                className="w-full bg-white/[0.03] backdrop-blur-md border border-white/10 focus:border-yellow-400/50 rounded-2xl py-4.5 pl-14 pr-20 text-white text-lg font-rajdhani outline-none transition-all placeholder:text-gray-500 shadow-2xl focus:shadow-[0_0_30px_rgba(234,179,8,0.1)]" />
              {query && (
              ...
              <button onClick={() => handleSearch(query)}
                className="absolute right-4 top-1/2 -translate-y-1/2 bg-yellow-500 hover:bg-yellow-400 text-black font-bold px-3.5 py-1.5 rounded-xl text-sm transition-all font-rajdhani">
                Go
              </button>
            </div>
                  ...
                  if (totalCount === 0 && suggestions.queries.length === 0) {
                    return (
                      <div className="flex flex-col items-center justify-center py-16 px-4 bg-white/[0.02] border border-white/5 rounded-3xl backdrop-blur-md shadow-2xl text-center max-w-md mx-auto">
                        <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4 text-2xl border border-white/10">
                          🔍
                        </div>
                  ...
                  return (
                    <div className="space-y-8 pb-10">
                      ...
                      {universeRes.length > 0 && (
                        <div>
                          <p className="text-gray-500 text-[10px] font-black tracking-widest uppercase mb-3 flex items-center gap-1.5"><span className="w-1.5 h-1.5 bg-yellow-400 rounded-full" /> Universes</p>
                          <div className="flex gap-2.5 flex-wrap">
                            {universeRes.map((uni: any) => (
                              <Link key={uni.id} href={`/universe/${uni.id}`} onClick={() => { addRecentSearch(uni.name); closeSearch() }}
                                className="px-4 py-2.5 rounded-xl border border-white/5 hover:border-yellow-400/30 font-bold text-xs tracking-wide transition-all hover:scale-[1.02] font-rajdhani flex items-center gap-2"
                                style={{
                                  background: `${uni.color || '#FFD700'}12`,
                                  borderColor: `${uni.color || '#FFD700'}25`,
                                  color: uni.color || '#FFD700'
                                }}>
                                <span>🌌</span>
                      ...
                      {moviesRes.length > 0 && (
                        <div>
                          <p className="text-gray-400 text-xs font-black tracking-widest uppercase mb-4 flex items-center gap-2 font-rajdhani"><span className="w-1.5 h-1.5 bg-red-500 rounded-full" /> Movies / సినిమాలు ({moviesRes.length})</p>
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      ...
                      {animeRes.length > 0 && (
                        <div>
                          <p className="text-gray-400 text-xs font-black tracking-widest uppercase mb-4 flex items-center gap-2 font-rajdhani"><span className="w-1.5 h-1.5 bg-purple-500 rounded-full" /> Anime / అనిమే ({animeRes.length})</p>
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            {animeRes.map((item, i) => (
                              <div key={item.id} onClickCapture={() => { addRecentSearch(item.titleEnglish); closeSearch() }} className="hover:scale-[1.03] transition-all">
                                <ContentCard content={item as any} index={i} size="sm" />
                      ...
                      {seriesRes.length > 0 && (
                        <div>
                          <p className="text-gray-400 text-xs font-black tracking-widest uppercase mb-4 flex items-center gap-2 font-rajdhani"><span className="w-1.5 h-1.5 bg-blue-500 rounded-full" /> Web Series & Shows / సిరీస్ ({seriesRes.length})</p>
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      ...
                      {suggestions.queries.length > 0 && (
                        <div className="border-t border-white/5 pt-5">
                          <p className="text-gray-500 text-[10px] font-black tracking-widest uppercase mb-3">🔍 Search Suggestions</p>
                ...
                {recentSearches.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-3 border-b border-white/5 pb-2">
                      <p className="text-gray-400 text-xs font-bold font-rajdhani tracking-widest uppercase flex items-center gap-2"><span className="text-yellow-400">🕐</span> Recent Searches</p>
                ...
                {trendingSearches.length > 0 && (
                  <div>
                    <div className="mb-3 border-b border-white/5 pb-2">
                      <p className="text-gray-400 text-xs font-bold font-rajdhani tracking-widest uppercase flex items-center gap-2"><span className="text-red-500">🔥</span> Trending Searches</p>
                    </div>
                    <div className="space-y-1">
